...
// ─────────────────────────────────────────────
//  Hardware configuration — edit these freely
// ─────────────────────────────────────────────
#define SDA_PIN        21
#define SCL_PIN        22
#define TOUCH_PIN       4
...
static float easeOut(float t) { float f = 1.0f - t; return 1.0f - f * f; }
...
static void drawZ(int x, int y, int sz) {
    // Top bar
    for (int i = 0; i < sz; i++) u8g2.drawPixel(x+i, y);
    // Diagonal
    for (int i = 0; i < sz; i++) u8g2.drawPixel(x+sz-1-i, y+i*(sz/sz));
    // Bottom bar
    for (int i = 0; i < sz; i++) u8g2.drawPixel(x+i, y+sz-1);
}

// Approximate Zzz with a small pixel Z
static void drawZLetter(int x, int y, int size) {
    // top
    for (int i = 0; i < size; i++) { u8g2.drawPixel(x+i, y); }
    // diagonal
    for (int i = 0; i < size; i++) { u8g2.drawPixel(x+size-1-i, y+i); }
    // bottom
    for (int i = 0; i < size; i++) { u8g2.drawPixel(x+i, y+size-1); }
}
...
static void renderFace(const AnimState& a, float globalMs) {
    ...
    if (a.showZzz) {
        ...
        if (a.zzzPhase >= 0) { drawZLetter(zx,     zy + 0,  4); }
        if (a.zzzPhase >= 1) { drawZLetter(zx + 2, zy - 5,  5); }
        if (a.zzzPhase >= 2) { drawZLetter(zx + 4, zy - 11, 6); }
    }
    if (a.showDizzy) {
...
static void updateBlink(AnimState& a, float ms) {
    ...
    a.t += a.phaseSpeed;
    if (a.t >= 1.0f) { a.t = 0.0f; advancePhase(a); }

    float close = 0.0f;
    switch (a.phase) {
        case 0: close = easeIn(a.t);         a.phaseSpeed = 0.14f; break;
        case 1: close = 1.0f;                a.phaseSpeed = 0.5f;  break;
        case 2: close = easeOut(1.0f - a.t); a.phaseSpeed = 0.10f; break;
        default: a.eyeScaleY = 1.0f; return;
    }
...
static void updateAngry(AnimState& a, float ms) {
    ...
    switch (a.phase) {
        case 0: // Eyes shrink and brows tilt
            ...
            a.phaseSpeed = 0.05f; break;
        case 1: // Shake
            a.shake     = 1.0f;
            a.phaseSpeed = 0.018f; break;
        case 2: // Hold angry
...
static void updateExpression(Expression e, AnimState& a, float ms) {
    switch (e) {
        case IDLE:       updateIdle(a, ms);       break;
        case BLINK:      updateBlink(a, ms);      break;
        case HAPPY:      updateHappy(a, ms);      break;
        case SAD:        updateSad(a, ms);        break;
        case ANGRY:      updateAngry(a, ms);      break;
        case SURPRISED:  updateSurprised(a, ms);  break;
        case TIRED:      updateTired(a, ms);      break;
        case CURIOUS:    updateCurious(a, ms);    break;
        case CONFUSED:   updateConfused(a, ms);   break;
        case LAUGHING:   updateLaughing(a, ms);   break;
        case LOVE:       updateLove(a, ms);       break;
        case WINK:       updateWink(a, ms);       break;
        case SCARED:     updateScared(a, ms);     break;
        case SUSPICIOUS: updateSuspicious(a, ms); break;
        case DIZZY:      updateDizzy(a, ms);      break;
        case SLEEP:      updateSleep(a, ms);      break;
        default: break;
    }
}
...
static void initExpression(Expression e, AnimState& a) {
    switch (e) {
        case IDLE:       initIdle(a);       break;
        case BLINK:      initBlink(a);      break;
        case HAPPY:      initHappy(a);      break;
        case SAD:        initSad(a);        break;
        case ANGRY:      initAngry(a);      break;
        case SURPRISED:  initSurprised(a);  break;
        case TIRED:      initTired(a);      break;
        case CURIOUS:    initCurious(a);    break;
        case CONFUSED:   initConfused(a);   break;
        case LAUGHING:   initLaughing(a);   break;
        case LOVE:       initLove(a);       break;
        case WINK:       initWink(a);       break;
        case SCARED:     initScared(a);     break;
        case SUSPICIOUS: initSuspicious(a); break;
        case DIZZY:      initDizzy(a);      break;
        case SLEEP:      initSleep(a);      break;
        default: break;
    }
}
...
static void handleTouch(unsigned long now) {
    bool raw = digitalRead(TOUCH_PIN);   // TTP223: HIGH = touched
    if (raw && !lastTouchVal && (now - lastTouchMs) > TOUCH_DEBOUNCE) {
        lastTouchMs = now;
        // Advance expression
...
void loop() {
    ...
    float ms = (float)now;  // absolute time for oscillators

    // ── Transition logic ───────────────────────
    if (transitioning) {
        transitionT += 0.08f;   // ~8 frames = ~265 ms transition
        ...
        if (transitionT >= 1.0f) {
            ...
            // Scale down / scale up transition
            // Eyes pinch to nothing, then new expression opens
            float squeeze = transitionT < 0.5f
                ? (1.0f - easeIn(transitionT * 2.0f))
                : easeOut((transitionT - 0.5f) * 2.0f);
            anim.eyeScaleX = squeeze;
            anim.eyeScaleY = squeeze;
            anim.bounce    = (1.0f - squeeze) * -3.0f;
        }
    } else {
        // ── Idle sub-events ──────────────────────
        if (currentExpr == IDLE) {
            ...
            if (now >= idleNextBlink) {
                // Do an inline blink via eyeScaleY oscillation
                // The idle updater handles breathe; blink layered on top
                float blinkPhase = fmodf((now - idleNextBlink) / 200.0f, 1.0f);
                // Simple V shape: 0→close→open over 200ms
                float c = (blinkPhase < 0.5f)
                    ? easeIn(blinkPhase * 2.0f)
                    : easeOut((1.0f - blinkPhase) * 2.0f);
                anim.eyeScaleY = 1.0f - 0.9f * c;
                if (blinkPhase > 0.95f) {
                    anim.eyeScaleY = 1.0f;
                    scheduleIdleBlink(now);
                }
            ...
            if (now >= idleNextLook) {
                idleGazeTarget = (float)random(-3, 4) / 3.0f;
                scheduleIdleLook(now);
            }
            // Smooth gaze interpolation
            idleGazeX += (idleGazeTarget - idleGazeX) * 0.06f;
        }

        // ── Update current expression ──────────────
        updateExpression(currentExpr, anim, ms);
    }
